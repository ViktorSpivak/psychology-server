  ...
  try {
    ...
    if (!isEmail) {
      ...
      const msg = {
        ...
        html: `<p>For complete verification enter the code ${otpCode}</p><form action='psychology-server.herokuapp.com/auth/otp?email=${email}' method="post"><input  name="otpCode" placeholder="Enter code"></input><button type="submit">Click to confirm</button></form>`,
      };
      sendEmail(msg);
